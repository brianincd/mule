...
import org.mule.extension.email.internal.mailbox.DummyResolver;
 ...
 * @since 4.0
 */
@MetadataScope(outputResolver = DummyResolver.class)
public class IMAPOperations {
   ...
   * @return an {@link OperationResult} {@link List} carrying all the emails content and it's corresponding
   *         {@link IMAPEmailAttributes}.
   */
  @Summary("List all the emails in the given POP3 Mailbox Folder")
  public PagingProvider<MailboxConnection, OperationResult<Object, IMAPEmailAttributes>> listImap(@UseConfig IMAPConfiguration config,
    ...
    return new PagingProviderEmailDelegate<>(config, mailboxFolder, imapMatcher, pageSize, deleteAfterRetrieve,
                                             attributes -> markAsDeleted(connection, mailboxFolder, attributes.getId()),
                                             () -> {
